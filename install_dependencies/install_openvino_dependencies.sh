...
if [ "$os" == "auto" ] ; then
    ...
    # Map some derivatives (Linux Mint) to their Ubuntu base so installer
    # can reuse the Ubuntu package selection logic. For example, Linux Mint
    # 22.x is based on Ubuntu 22.04 and should be treated as ubuntu22.04.
    if [ -f /etc/os-release ]; then
        ...
        if [ "${ID}" = "linuxmint" ]; then
            case ${VERSION_ID} in
                22.*) os="ubuntu22.04" ;;
                21.*) os="ubuntu22.04" ;; # Linux Mint 21 is also based on Ubuntu 22.04
                20.*) os="ubuntu20.04" ;;
                *) os="ubuntu22.04" ;; # conservative default mapping
            esac
        fi
    fi
    case $os in
